...
void Model::load(IMAGE& bk, IMAGE mybird[][2], IMAGE guid[], IMAGE up[], IMAGE down[], IMAGE start[], IMAGE end[])
{
	...
	// 加载鸟的图片
	loadimage(mybird[0], "./resource/picture/bird0.bmp", BRID_SIZE, BRID_SIZE);
	loadimage(mybird[0] + 1, "./resource/picture/birdy0.bmp", BRID_SIZE, BRID_SIZE);
	loadimage(mybird[1], "./resource/picture/bird1.bmp", BRID_SIZE, BRID_SIZE);
	loadimage(mybird[1] + 1, "./resource/picture/birdy1.bmp", BRID_SIZE, BRID_SIZE);
	loadimage(mybird[2], "./resource/picture/bird2.bmp", BRID_SIZE, BRID_SIZE);
	loadimage(mybird[2] + 1, "./resource/picture/birdy2.bmp", BRID_SIZE, BRID_SIZE);
	// 加载下方向柱子图片
...
void Model::moveBird()
{
	...
	// 对鸟的移动边界进行限制
	if (this->bird.y >= 607.0 - BRID_SIZE + 15.0)
	{
		this->bird.y = 607.0 - BRID_SIZE + 15.0;
	}
...
int Model::hitPillar()
{
	for (int i = 0; i < PILLAR_NUM; i++)
	{
		if (this->pillar[i].x <= this->bird.x + BRID_SIZE - 15 && this->pillar[i].x >= this->bird.x - 65 + 10)
		{
			// 判断上柱是否发生碰撞
			...
			// 判断下柱是否发生碰撞
			if (this->bird.y + BRID_SIZE - 15 >= 607 - (400.0 - this->pillar[i].height))
			{
				return 1; // 返回值为1说明有碰撞发生
			}
		}
...
void Model::hitAction(IMAGE& bk, IMAGE up[], IMAGE down[], IMAGE mybird[][2])
{
	...
	// 绘制动画效果
	while (this->bird.y + BRID_SIZE - 15 <= 607)
	{
		this->bird.y += step;
		step += 0.025;
